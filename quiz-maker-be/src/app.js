...
function createApp() {
	...
	app.post("/quizzes/:id/questions", (req, res) => {
		try {
			...
			for (let i = 0; i < items.length; i++) {
				const { type, prompt, options, correctAnswer } = items[i];
				if (!type || !prompt)
					return res.status(400).json({ error: `item ${i}: type and prompt are required` });
				if (!["mcq", "short", "code"].includes(type))
					return res.status(400).json({ error: `item ${i}: invalid type` });
				if (type === "mcq") {
					if (!Array.isArray(options) || options.length < 2)
						return res.status(400).json({ error: `item ${i}: mcq requires options (>=2)` });
					if (correctAnswer === undefined || correctAnswer === null)
						return res.status(400).json({ error: `item ${i}: mcq requires correctAnswer (index or text)` });
				} else if (type === "short") {
					if (
						correctAnswer === undefined ||
						(typeof correctAnswer !== "string" && correctAnswer !== null)
					)
						return res.status(400).json({ error: `item ${i}: short requires correctAnswer (string)` });
				}
			}
        ...
        VALUES (?,?,?,?,?,?,?)`);
			...
			for (const item of items) {
				const { type, prompt, options, correctAnswer, position } = item;
	...
	app.post("/attempts/:id/answer", (req, res) => {
		try {
			...
			for (let i = 0; i < items.length; i++) {
				const { questionId, value } = items[i];
				if (!questionId || value === undefined || value === null)
					return res.status(400).json({ error: `item ${i}: questionId and value required` });

				const q = db
					.prepare(`SELECT * FROM questions WHERE id=?`)
					.get(Number(questionId));
				if (!q) return res.status(404).json({ error: `item ${i}: Question not found` });
				if (q.quiz_id !== att.quiz_id)
					return res
						.status(400)
						.json({ error: `item ${i}: Question does not belong to this attempt's quiz` });
			}

			const stmt = db.prepare(`INSERT INTO attempt_answers (attempt_id, question_id, value) VALUES (?,?,?)
        ON CONFLICT(attempt_id, question_id) DO UPDATE SET value=excluded.value`);

			for (const { questionId, value } of items) {
				stmt.run(
					attemptId,
					Number(questionId),
					String(value),
